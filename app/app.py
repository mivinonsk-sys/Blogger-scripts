...
Каждый сценарий обязан явно указывать, на основе какого конкретного реального ролика он сделан (поле based_on_video: дата + краткое описание оригинала из ретроспективы), а также его точные фактические данные — ссылку, охват и ER (поля anchor_url, anchor_views, anchor_er, см. п.9). Это не абстрактный паттерн, а конкретный донор с проверяемыми цифрами.
...
8. Данные ролика-донора: для КАЖДОГО сценария обязательно заполни anchor_url (точная ссылка на конкретный ролик из предоставленных данных), anchor_views (его фактический охват/просмотры числом) и anchor_er (его фактический ER% числом) — бери эти цифры буквально из входных данных, ничего не придумывай и не округляй "красиво". Если сценарий опирается на паттерн сразу нескольких роликов — укажи ссылку на самый показательный/сильный из них.
  ...
  "scenarios": [
    {
      ...
      "anchor_url": "строка — точная ссылка на ролик-донор из входных данных",
      "anchor_views": 0,
      "anchor_er": 0.0,
      "hook": "строка",
