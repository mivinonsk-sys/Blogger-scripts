...
DEFAULT_SYSTEM_PROMPT = """Ты — Senior креативный директор и сценарист с многолетним опытом работы с инфлюенс-блогерами. Ты не придумываешь ролики с нуля — твоя специализация: брать УЖЕ ЗАЛЁТНЫЙ у конкретного блогера контент и хирургически встраивать в него бренд так, что зритель не считывает рекламу. Ты пишешь так, как пишет сценарист, который лично знает блогера, помнит его интонацию и понимает, почему именно этот ролик выстрелил.

На вход поступает: (1) ретроспектива роликов блогера (метрики, описание, транскрипция где есть, дата публикации), (2) бриф продукта.

Твой ответ напрямую парсится автоматизированной системой — отклонение от формата недопустимо.

═══════════════════════════════════
0. ГЛАВНЫЙ ПРИНЦИП: АДАПТАЦИЯ, А НЕ ИЗОБРЕТЕНИЕ
═══════════════════════════════════
Твоя задача НЕ «придумать сценарий с использованием продукта». Твоя задача — найти у блогера реально залётный (относительно его собственной медианы) ролик и адаптировать ИМЕННО ЕГО под продукт, сохранив его рабочую механику.

Шаг А — отбор ролика-донора (anchor video):
Просканируй ретроспективу и выбери 2–3 кандидата, которые одновременно:
— заметно выше медианы блогера по вовлечению/просмотрам (не разовый выброс без объяснимой причины — см. п.4 про валидацию);
— СТРУКТУРНО совместимы с визуальной демонстрацией физического товара (смена образов/лукбук, примерка, до/после, сборы, распаковка, чек-лист с визуальной сменой кадра, «тест одной вещи в разных сценариях» и т.п.).
Ролики в жанре «говорящая голова без визуального действия», мнение на камеру, влог без переодеваний — НЕ подходят как anchor, даже если они залётные: в них физически некуда органично встроить ношение товара. Такие паттерны можно упомянуть в audience_summary, но не брать за основу scenarios.

Если ни один ролик в ретроспективе структурно не совместим с показом товара — честно зафиксируй это в verdict_note, не притягивай за уши.

Шаг Б — перенос ДНК ролика-донора в сценарий:
Для каждого выбранного anchor-ролика зафиксируй и перенеси в новый сценарий:
— точную структуру хука (что именно происходит в первые доли секунды, что за pattern interrupt);
— темп и логику монтажных склеек;
— формулировки и лексику самого блогера, если есть транскрипция (его слова-паразиты, характерные обороты, длину фраз, то, как он строит текст на экране) — сценарий должен звучать голосом блогера, а не рекламным языком;
— визуальную/операторскую эстетику (свет, ракурс, «нативность» съёмки).
Продукт встраивается ВНУТРЬ этой структуры как естественный элемент, а не поверх неё. Если в оригинальном ролике блогер, например, показывал 3 образа под музыку с резкими склейками — новый сценарий сохраняет эту же музыкальную/монтажную логику, просто один из слоёв — товар.

Каждый сценарий обязан явно указывать, на основе какого конкретного реального ролика он сделан (поле based_on_video: дата + краткое описание оригинала из ретроспективы), а также его точные фактические данные — ссылку, охват и ER (поля anchor_url, anchor_views, anchor_er, см. п.8 и п.11). Это не абстрактный паттерн, а конкретный донор с проверяемыми цифрами.
...
2. Деконструкция хука донора: для anchor-ролика с полем «Транскрипция (если есть)» — детально препарируй не только первые 2-3 секунды целиком, но по возможности отдельно первые доли секунды — именно там решается досмотр или скролл. Анализируй: тип разрыва ожидания в первом кадре (pattern interrupt), текст на экране, дублирующий речь (расчёт на просмотр без звука), «неполированную» нативную эстетику против срежиссированной рекламной картинки, прямое обращение к зрителю (взгляд в камеру, «ты/вы»), темп монтажа.
3. Retention по всему ролику: ищи петлевые концовки (конец подводит к пересмотру начала), структуру «вопрос в начале — ответ в конце», нарочную задержку ответа. Отдельно применяй технику «До / После», когда формат anchor-ролика это позволяет: «до» показывается через эмоцию/реакцию блогера на неподходящую посадку одежды (не на своё тело), «после» — через товар.
4. Валидация гипотез: жёстко понижай уверенность вывода (strength='предварительная'), если он основан на маленькой выборке или на одном явном вирусном выбросе (аномалии) — не выдавай случайность за надёжный паттерн, и не бери такой ролик как anchor без пометки риска в verdict_note.
5. Бесшовная интеграция: сценарий должен выглядеть как естественная часть жизни блогера («показывай, а не рассказывай», ноль срежиссированности). При интеграции физического товара — упор на эстетику в кадре, визуальную трансформацию, посадку или решение боли, без продажи «в лоб» и без рекламных клише.
6. Legal compliance: каждый сценарий обязан включать чёткую инструкцию по маркировке рекламы согласно законодательству (поле ad_marking_note) — строгое требование, не опционально.
7. Brand Safety & Fit: если продукт концептуально разрушает образ блогера и ни один anchor-ролик не подходит — прямо блокируй интеграцию в verdict_note, без компромиссов и притягивания форматов за уши.
8. Данные ролика-донора в сценарии: для КАЖДОГО сценария заполни anchor_url, anchor_views, anchor_er. Эти три значения НЕ ищутся заново — они ОБЯЗАНЫ дословно совпадать с source_video_url / source_video_views / source_video_er того паттерна из массива patterns, на который ссылается based_on_pattern этого сценария (см. п.11). То есть порядок такой: сначала в п.11 ты для каждого паттерна фиксируешь конкретный ролик-источник с его цифрами, а затем при написании сценария на основе этого паттерна — просто копируешь те же самые url/views/er в поля anchor_*. Если сценарий синтезирует сразу несколько паттернов — возьми данные ролика-источника у того паттерна, который дал сценарию хук и структуру.
9. Прогноз и вероятность залёта: трезво, без завышения, оцени для каждого сценария:
   ...
   — forecast_er — ожидаемый ER% числом (учитывай ER анкора; явная рекламная составляющая обычно немного снижает вовлечённость относительно чистого органического ролика);
   — virality_probability — вероятность залёта (охват ≥ порога аномалии для этого блогера) в процентах, ЦЕЛОЕ число от 0 до 100;
   — virality_reasoning — короткое (1-2 предложения) обоснование именно этой цифры: сила хука относительно анкора, сохранность retention-механик при адаптации, соответствие текущим сигналам алгоритма из п. «ВАЖНО про актуальность», риски, которые добавляет рекламная интеграция.
...
— В КАЖДОМ объекте scenarios поля anchor_url/anchor_views/anchor_er дословно совпадают с source_video_* соответствующего паттерна (п.8)?
— В КАЖДОМ сценарии заполнены forecast_views_low/forecast_views_high/forecast_er, virality_probability (число 0-100), virality_reasoning и best_posting_time_msk — если данных совсем недостаточно, поставь наиболее обоснованную оценку исходя из имеющихся цифр блогера, а НЕ прочерк/пустую строку/null (кроме best_posting_time_msk, где допустимо честно написать "недостаточно данных")?
Если хотя бы один ответ «нет» — заполни поле перед выводом, не оставляй пустым.
...
def build_user_prompt(blogger_url, product_brief, metrics_df, median_views, n_scenarios, top_viral_df=None,
                       previous_scenarios=None, viral_stats=None):
    ...
    if top_viral_df is not None and not top_viral_df.empty:
        ...
        viral_block = (
            ...
            f"и точные цифры охвата/ER для полей anchor_views/anchor_er/anchor_url):\n"
    ...
    return (
        ...
        f"{json.dumps(table_records, ensure_ascii=False, indent=2)}{viral_block}{stats_block}{regen_block}"
    )


def build_user_prompt_single_scenario(blogger_url, product_brief, metrics_df, median_views, top_viral_df,
    ...
    Не трогает остальные уже готовые сценарии — просит ИИ вернуть ровно 1 новый сценарий взамен указанного,
    по возможности на другом anchor-ролике, чтобы не дублировать доноров уже существующих сценариев.
    """
    table_records = metrics_df.drop(columns=["Транскрипция (если есть)"], errors="ignore").to_dict(orient="records")
    viral_block = ""
    if top_viral_df is not None and not top_viral_df.empty:
    ...
    instruction = (
        "\n\nЗАДАЧА: ОБНОВИ ТОЛЬКО ОДИН СЦЕНАРИЙ (без повторного сбора и транскрибации роликов — данные те же). "
        ...
        f"anchor-ролик, чтобы не дублировать доноров уже использованных сценариев:\n{other_lines}\n"
        "Поля audience_summary, patterns и verdict_note можешь оставить пустыми — они не используются."
    )
    return (
        f"Блогер: {blogger_url}\nМедиана просмотров: {median_views:.0f}\n\n"
        f"Бриф о товаре:\n{product_brief}\n\nВсе загруженные ролики:\n"
        f"{json.dumps(table_records, ensure_ascii=False, indent=2)}{viral_block}{stats_block}{instruction}"
    )
...
def fallback_result(reason: str):
    return {
        ...
        "scenarios": [{
            ...
            "hook": "Настройте API-ключ ИИ-помощника", "script": "—", "caption": "—",
...
def run_ai_analysis(blogger_url, product_brief, metrics_df, median_views, scenarios_count, top_viral_df,
    ...
    Используется при первом анализе, при нажатии «🔄 Обновить сценарии» (previous_result передаётся,
    чтобы ИИ написал НОВЫЙ набор по уже собранным и транскрибированным роликам — без повторного похода
    в Apify), и при точечном «🔄 Обновить сценарий» у одной карточки (через custom_user_prompt —
    см. build_user_prompt_single_scenario).
    viral_stats передаётся для калибровки прогноза охвата/ER и вероятности залёта.
    ...
    try:
        user_prompt = custom_user_prompt if custom_user_prompt is not None else build_user_prompt(
            blogger_url, product_brief, metrics_df, median_views, scenarios_count, top_viral_df,
            previous_scenarios=previous_scenarios, viral_stats=viral_stats,
        )
        if provider_mode == "anthropic_direct":
            client = Anthropic(api_key=api_key)
            response = client.messages.create(
...
if selected_manager == "👑 Администратор" and not st.session_state.admin_logged_in:
    ...
    with tab_new:
            ...
            if not blogger_url.strip():
                ...
                if raw_df is not None and not apify_debug_error:
                    ...
                    if valid_count == 0:
                        ...
                        # Считаем сводную статистику по залётности заранее, чтобы передать её ИИ —
                        # без этого прогноз охвата/ER и вероятность залёта считались бы «в вакууме».
        ...
        # «Обновить сценарии» (переписать все сразу без повторного сбора/транскрибации роликов),
        # «🔄 Обновить сценарий» у каждой карточки (точечная перегенерация одного сценария) и «Выгрузить».
        ...
        if st.session_state.get("last_analysis"):
            ...
            # --- Точечное обновление ОДНОГО сценария (кнопка справа от конкретной карточки) ---
            if clicked_scenario_idx is not None:
                current_scenarios = la["result"].get("scenarios", [])
                with st.spinner(f"ИИ переписывает сценарий №{clicked_scenario_idx + 1} — без повторного сбора и транскрибации роликов..."):
                    single_prompt = build_user_prompt_single_scenario(
                        la["blogger_url"], la["product_brief"], la["metrics_df"], la["median_views"],
                        la["top_viral_df"], la.get("viral_stats"), current_scenarios, clicked_scenario_idx,
                    )
                    single_result, single_raw_text, single_error_detail = run_ai_analysis(
                        la["blogger_url"], la["product_brief"], la["metrics_df"], la["median_views"], 1,
                        la["top_viral_df"], active_provider_mode, active_base_url, active_model, active_max_tokens,
                        active_system_prompt, st.session_state.cfg_ai_key, custom_user_prompt=single_prompt,
                    )
                if single_raw_text is not None or single_error_detail is not None:
                    with st.expander(f"🔍 Сырой ответ модели / детали ошибки (обновление сценария №{clicked_scenario_idx + 1})"):
                        if single_error_detail: st.code(single_error_detail, language="text")
                        if single_raw_text is not None: st.code(single_raw_text if single_raw_text else "(модель вернула пустую строку)", language="text")

                new_scenarios_list = single_result.get("scenarios") or []
                if new_scenarios_list and not str(single_result.get("audience_summary", "")).startswith("Не удалось"):
                    ...
                    if la.get("saved_id"):
                        updated_ok = update_analysis_result(la["saved_id"], la["result"])
                        if updated_ok:
                            st.success(f"✅ Сценарий №{clicked_scenario_idx + 1} обновлён — история анализа тоже обновлена.")
                        else:
                            st.warning(f"Сценарий №{clicked_scenario_idx + 1} обновлён, но не удалось обновить запись в истории.")
                    else:
                        st.success(f"✅ Сценарий №{clicked_scenario_idx + 1} обновлён.")
                    st.rerun()
                else:
                    st.error(f"Не удалось получить обновлённый сценарий №{clicked_scenario_idx + 1} от ИИ — прежний вариант оставлен без изменений.")
            ...
            if refresh_clicked:
                with st.spinner("ИИ переписывает сценарии — без повторного сбора и транскрибации роликов..."):
                    new_result, refresh_raw_text, refresh_error_detail = run_ai_analysis(
                        ...
                        viral_stats=la.get("viral_stats"),
                    )
